...
(function ($) {
 Backdrop.behaviors.draggableViews = {
  attach: function (context, settings) {
    $(Backdrop.settings.draggableviews_row_class).each(function(index, row_class ) {
      ...
      .sortable({
        update: function (event, ui) {
          ...
          if (!$(this).hasClass('draggableviews-changed')) {
            ...
            if (!Backdrop.settings.draggableviews_ajax) {
              $('<div class="draggableviews-changed-warning messages warning">' + Backdrop.t('Changes made in this list will not be saved until the form is submitted.') + '</div>')
    ...
    if (Backdrop.settings.draggableviews_ajax) {
      $('.views-form .' + row_class).parent().parent().find('.form-actions input[id^="edit-submit"]').hide();
    }
    });
  }
 }
